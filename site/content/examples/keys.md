+++
title = "Keys"
description = "Keyed storage. Declared columns are the composite key, in order, and may only be int or string. Inserts must supply every declared key field with the declared type; the physical key is an order-preserving encoding of the key columns, so scans return rows in key order. Keyless tables keep surrogate ids."
weight = 11
+++
